...
import {
  isLoadFailedPayload, isPdfPayload, type LoadFailedPayload, type PdfLoadFailure,
} from './shared.js';
...
export function activate(): TabPluginActivation {
  return {
    ...
    intent: defineIntents('pdf', isPdfPayload, {
      'load-failed': {
        payload: isLoadFailedPayload,
        run: (tabPayload, payload: LoadFailedPayload, capabilities) => {
