...
export class ControllerCore {
  ...
  // The metadata row's detach/attach control. One method behind two verbs, delegating to the same
  ...
  remoteSession(action: 'detach' | 'attach', label: string): boolean {
    if (action === 'detach') return this.managers.sessions.detach(label);
    return this.managers.sessions.attachTab(label);
  }
