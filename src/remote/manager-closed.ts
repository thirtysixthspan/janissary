 ...
 * limit, and written over the entry table it mutates rather than over the manager, so the one thing
 * it needs from the manager is visible in its signature.
 *
 * An established session — one with a session id and a workspace — treats this as a lost transport
 * and hands it to the recovery backoff instead, because the peer on the far side is still there
 * holding both. Everything below that branch is the genuine end: readiness is settled or rejected,
 * the workspace's local file cache goes, the entry leaves the table, and every tab and navigator
 * holding it hears `onClosed`.
 */
