...
const MISSING = '/nonexistent-janissary-playwright-placeholder';
...
function resolveDir(specifier: string): string | undefined {
  try {
...
export function playwrightPackagePaths(): PlaywrightPackagePaths {
  ...
  const dirs = ['playwright', 'playwright-core'].map((name) => resolveDir(name) ?? MISSING);
...
function findChromiumBundleDir(): string {
  ...
  const resolved = existsSync(executable) ? realpathSync(executable) : executable;
