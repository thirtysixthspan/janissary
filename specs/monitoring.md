...
An **editor-view target** (see [[editor-tab]]) also has no `LogEntry` transcript, so it instead contributes its file content read from disk. The content is read at monitor start and again on each 30-second flush, but only fed when it has changed since the last read. The first feed to a given monitor is the full current content; every feed after that is a size-capped **unified diff** against what was last fed to *that* monitor, so a monitor watching a large file accumulates roughly one edit's worth of change per flush rather than the whole file each time. Any single entry — the first full-content one included — is truncated with a trailing note if it would exceed the cap. Because the content is read from disk, an unsaved edit sitting in the editor buffer is not visible to a monitor until the file is actually saved; a save (or an external change on disk) is what the next flush picks up, and a file deleted from disk feeds a diff that removes every line.
