...
describe('renderLine — prompt click', () => {
  it('calls onPromptClick with the line text when a plain prompt is double-clicked', async () => {
    ...
    render(<>{renderLine(line, 0, intentsStub, noop, onPromptClick)}</>);
  ...
  it('calls onToggleCollapse and not onPromptClick when an ACP prompt is clicked', async () => {
    ...
    render(<>{renderLine(line, 0, intentsStub, onToggleCollapse, onPromptClick)}</>);
  ...
  it('does not call onPromptClick when the cwd text is double-clicked, only the command text', async () => {
    const onPromptClick = vi.fn();
    const line = makePromptLine({ text: 'git status', cwd: '/home/user' });
    const { container } = render(<>{renderLine(line, 0, intentsStub, noop, onPromptClick)}</>);
...
describe('renderLine — markdown link click', () => {
  it('asks to open an https link clicked in markdown', async () => {
    const intents = fakeIntents();
    const line = makeMarkdownLine('Here is [a link](https://example.com) for you');
    render(<>{renderLine(line, 0, intents, noop, vi.fn())}</>);
  ...
  it('asks to edit a file:line link clicked in markdown', async () => {
    const intents = fakeIntents();
  ...
  it('asks for nothing when plain text without a link is clicked', async () => {
    const intents = fakeIntents();
...
describe('renderLine — ansi output', () => {
  ...
  it('gives priority to the search hit over ansi styling', () => {
    ...
    const { container } = render(<>{renderLine(line, 0, intentsStub, noop, vi.fn(), { lineIndex: 0, pattern: 'error' })}</>);
...
describe('renderLine — search highlight', () => {
  ...
  it('does not highlight a line other than the current match', () => {
    const line: BufferLine = { type: 'output', text: 'an error occurred' };
    const { container } = render(<>{renderLine(line, 1, intentsStub, noop, vi.fn(), { lineIndex: 0, pattern: 'error' })}</>);
...
describe('renderLine — message openFile link', () => {
  it('renders a clickable link that asks to edit the capture when the message carries openFile', async () => {
    ...
    expect(intents.onEditFile).toHaveBeenCalledWith('/captures/claude-now.txt');
  });
...
describe('renderLine — message tab link', () => {
  it('asks to focus the owning tab when its notification label is clicked', async () => {
    const intents = fakeIntents();
    ...
    expect(intents.onFocusTab).toHaveBeenCalledWith('build');
  });
});
