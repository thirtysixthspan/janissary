...
export function useServerState(client: JanusClient, setters: Setters): void {
  const {
    setTabs, setActiveTab, setRoute, setHarnessLaunch, setTabNameMaxLength, setGlobalHistory, setSyntaxTheme, setTheme, setTasks, setProfiles, setRouteIndex, routeRef,
  } = setters;
  const [projectDir, setProjectDir] = useState('');
  const [version, setVersion] = useState('');
  useProjectTitle(projectDir, version);
  useEffect(() => client.onState((nextTabs, active, nextRoute, nextTabNameMaxLength, nextGlobalHistory, nextSyntaxTheme, nextTheme, nextTasks, nextProfiles, nextProjectDir, nextVersion, nextHarnessLaunch) => {
    setTabs(nextTabs);
    setActiveTab(active);
    setRoute(nextRoute);
    ...
    client, setTabs, setActiveTab, setRoute, setHarnessLaunch, setTabNameMaxLength, setGlobalHistory, setSyntaxTheme, setTheme, setTasks, setProfiles, setRouteIndex, routeRef,
  ]);
}
