...
type StateListener = (tabs: TabView[], activeTab: number, route: RouteChooserView | null, tabNameMaxLength: number, globalHistory: string[], syntaxTheme: string, theme: string, tasks: TaskRow[], profiles: string[], projectDir: string, version: string, harnessLaunch: HarnessLaunchView | null) => void;
...
export class JanusClient {
  ...
  private onEvent(event: ServerEvent): void {
    ...
    case 'state': {
      ...
      for (const l of this.stateListeners) l(event.tabs, event.activeTab, event.route ?? null, event.tabNameMaxLength, event.globalHistory, event.syntaxTheme, event.theme, event.tasks, event.profiles, event.projectDir, event.version, event.harnessLaunch ?? null);
