...
type StateListener = (tabs: TabView[], activeTab: number, route: RouteChooserView | null, tabNameMaxLength: number, globalHistory: string[], syntaxTheme: string, tasks: TaskRow[], projectDir: string) => void;
...
export class JanusClient {
  ...
  private onEvent(event: ServerEvent): void {
    ...
    case 'state': {
      ...
      for (const l of this.stateListeners) l(event.tabs, event.activeTab, event.route ?? null, event.tabNameMaxLength, event.globalHistory, event.syntaxTheme, event.tasks, event.projectDir);
