...
describe('useCmdWRefs', () => {
  it('mirrors the plain arguments into their matching refs', () => {
    const { result } = renderHook(() => useCmdWRefs(2, false, false, false, false, false, false, null));
  ...
  it('ORs quitConfirmOpen with unsavedQuitOpen', () => {
    const { result } = renderHook(() => useCmdWRefs(0, false, true, false, false, false, false, null));
  ...
  it('ORs pickerOpen with queueOpen, taskPickerOpen, and profilePickerOpen', () => {
    const { result } = renderHook(() => useCmdWRefs(0, false, false, false, false, false, true, null));
  ...
  it('updates ref values on rerender without changing ref identity', () => {
    const { result, rerender } = renderHook(
      ({ activeTab }) => useCmdWRefs(activeTab, false, false, false, false, false, false, null),
