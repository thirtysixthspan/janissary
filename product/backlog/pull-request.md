...
* Make the post-write verification work for the first review on a branch, where the backlog file is newly created and therefore invisible to the commands the step names.

Existing Issue: The verification step tells the reader to confirm that `git diff` shows only appended lines inside the development section and to revert anything else with `git checkout -- <file>`, but both commands operate only on tracked files, and on the most common path — the first review on a branch, which creates the backlog file — the file is untracked, so `git diff` reports nothing at all and `git checkout --` cannot remove a stray. Severity: 5/10

Existing Risk: 5/10 - An agent following the step literally sees an empty `git diff` on a run that wrote several entries and either concludes the write failed and rewrites it, or treats the vacuous check as passing and commits without ever having verified what landed in the file.

Proposal Risk: 2/10 - The check covers both the tracked and untracked cases, but it is still a prose instruction an agent can skip rather than something the commit path enforces, so a run that ignores the step commits unverified exactly as it does today.

Proposal: Step 4's point 5 in `ai/tasks/pull-request-review.md` reads "`git status --porcelain` names `./product/backlog/pull-request.md` and nothing else, and `git diff` shows only appended lines inside `## development`", followed by the revert instruction. Split it into the two cases the porcelain status already distinguishes: when the status line begins `??` the file is new, so verify it by reading it back and confirming it carries the four standard headings plus the new entries and nothing else; when it begins with a modification marker, `git diff` is the right check and the existing wording applies. Replace the blanket `git checkout -- <file>` revert with one that names both remedies — `git checkout -- <file>` for a tracked file that was modified, and deleting the file for an untracked stray — since the current sentence names a command that cannot do what the sentence claims for half the cases it covers. This matters because the very next step runs `pr-commit`, which stages everything with `git add -A`: an untracked stray that survives the revert is swept into the review's commit, breaching the task's own tenth forbidden rule. The same flawed wording appears in the plan at `product/plans/complete/pull-request-review.md` under Step 4; leave it there as the historical record and fix only the task file, which is the executable copy.
