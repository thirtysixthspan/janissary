...
The extracted suites pass alone with the preserved totals: 36 browser-server cases, 18 remote-process cases, 45 sandbox cases, and 87 harness-manager cases. `./scripts/run.mjs check-diff` passes lint, typechecking, and the diff-selected server tests. A full `npm run test:server` also exercised the extracted suites but stopped on the unrelated existing `src/remote/serve.test.ts` case `walks up from the ssh login directory when given no argument`: macOS returned the real `/private/var/...` path while that test expected the symlinked `/var/...` spelling. The isolated case reproduces the same path-spelling failure without these changes.
