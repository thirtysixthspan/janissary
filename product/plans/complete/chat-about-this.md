...
**The draft travels through the tab payload.** The `conversations` `create` topic action and `ConversationsManager.create` take only an `id`. The plugin adds the selected text to the opened conversation tab's optional `draftQuery` field; neither the topic action nor the manager transports it.
...
**The conversations plugin's contribution and handler.** `src/plugins/conversations/manifest.ts` contributes `defaultMenu: { label: 'Chat about this' }`. The activation delegates creation to `src/plugins/conversations/tabs.ts`: a fresh id, an id-only `create` topic action, then `openOrFocusTab` with a factory that builds the tab payload including `draftQuery`. The result is the same tab the conversation list's create opens, plus the draft.

**Server: the tab owns the initial draft.** `ConversationTabPayload` (`src/plugins/conversations/shared.ts`) carries optional `draftQuery`; list payloads do not carry it. The plugin retains each initial draft in transient state and includes it in replacement payloads while the composer is still loading. The composer acknowledges capture through `consume-draft`, removing the retained value and its published initial field. Sending also clears it. Opening a closed conversation discards its draft, notifications prune closed instance keys, and plugin disposal releases all retained drafts. Nothing is written to ConversationStore. An ordinary list-created conversation omits the field.
...
**Client: the composer initializes from the draft.** `ConversationComposer` (`web/src/plugins/conversations/ConversationComposer.tsx`) accepts the payload's `draftQuery` as the initial value of its existing text state, only on mount. After capture it acknowledges the draft through ConversationTab's consumption intent. Subsequent payload updates preserve the composer's edited text.
...
- `src/plugins/conversations/activate.test.ts` — the default-menu handler emits one id-only `create` topic action and opens a tab titled `New conversation` whose payload carries the selected text in `draftQuery`. Ordinary list creation uses the same action without an initial draft. Notification and consumption tests cover the transient draft lifetime.
