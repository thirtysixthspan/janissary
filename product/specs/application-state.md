...
Only agent tabs are persisted and restored this way. View tabs — image, page, markdown, editor, file navigator, monitor, and notifications (see `tabs.md`) — are live, in-memory views that are never saved and never recreated on `--relaunch`. Harness and plugin tabs are not saved either. This holds for every kind of change: reordering, renaming or retargeting a non-agent tab, a scheduled command firing in it, or a new transcript entry in it never writes a state file.
