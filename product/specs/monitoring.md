...
Every target's fed content — agent transcript entries, harness screens, editor diffs, and page text alike — is wrapped in a random delimiter unique to that monitor's session when a flush prompt is assembled, and the persona is primed once at session start to treat anything between those markers as data from a monitored target, never as instructions. This defends against a monitored target's content trying to override the persona's own instructions (an indirect prompt injection); a page tab is the target most likely to carry adversarial content, since the app has no control over what a third-party site renders, but the same framing applies uniformly to every target type. This framing does not extend to `monitor ask` questions or their replies, which are user-authored, not monitored-target content.
