...
The restored harness displays its retained terminal output and transcript history without waiting for another response from the harness. A newly opened restored agent tab likewise shows its retained shell output in its transcript, without the shell's internal sentinel lines — restored history reads the way live command output always has. Earlier history can be trimmed to keep retention bounded; repeated reconnections do not duplicate the displayed history.
