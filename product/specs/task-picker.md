...
The task sets the workspace up by executing the project's own workspace preparation task — the project's copy when it has one, the app's own otherwise — and tests whatever branch that leaves checked out, without inspecting or rearranging the repository itself. Uncommitted work already in the tree is left where it is: the run never stashes it, and when it finds a change it cannot account for at commit time it stops rather than shipping or discarding it. Only one run works a project at a time: a second run stops before changing anything and names the run holding the lock, and a lock that outlives its run — a killed run — is cleared by hand. Web apps are exercised through the attached browser, and tools directly, with interactive behavior using an available pseudo-terminal. A web app is served on the loopback address only; a project that cannot be started that way is reported as an environment limitation and never started. Real user state and live app sessions are never used.
