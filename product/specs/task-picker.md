...
The task tests the remote's default branch, falling back to `master` when its default-branch reference is unset. Unpushed commits on the local primary branch stop the run before checkout. Uncommitted work, including untracked files, is stashed and restored onto the primary branch afterward; a conflicting restoration leaves the stash available and is reported. Only one run works a project at a time: a second run stops before changing anything and names the run holding the lock, and a lock that outlives its run — a killed run — is cleared by hand. Web apps are exercised through the attached browser, and tools directly, with interactive behavior using an available pseudo-terminal. A web app is served on the loopback address only; a project that cannot be started that way is reported as an environment limitation and never started. Real user state and live app sessions are never used.
