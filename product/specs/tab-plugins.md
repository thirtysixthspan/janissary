...
Plugin views that present the same shape share one plugin-wide look rather than each restating it: the metadata header above a body, the header's name, size, and location, its right-aligned action group, and the centering, scrollable stage the image and video views fill. Sharing it means those views stay consistent with each other as that look changes, and it loads once for however many of them are open. Anything one plugin alone draws — the image editor's crop overlay, the audio playlist, the markdown document area, the schedule list's columns — belongs to that plugin alone. A file size in that shared header stays on one line, so a narrow tab wraps the header between its parts rather than between a size and its unit.
...
A plugin may declare that it wants to be told when a named kind of host state changes, so a view can keep up with something the plugin does not own. This version defines three kinds: the set of scheduled commands, the conversation view, and the remote-session list. The conversation view contains the ordered summaries, loaded conversation windows, and selectable model pairs used by the conversations plugin; the remote-session list is the rows the sessions tab shows.
...
An announcement says only that something changed, so a plugin may also ask for that state directly — which is what a view opening for the first time needs, before anything has changed. It may likewise act on that state, through the small set of actions each kind defines. Scheduled-command actions cancel one entry, clear them all, or switch to the owning tab. Conversation actions create, load, extend, send, cancel, select a model, or delete. Remote-session actions detach, attach, end, forget, focus, or close one session, or re-read the list. All are limited to the kinds of state the plugin declared an interest in, so a plugin can only ever read or change something the application already agreed to show it, and asking about anything else disables it like any other broken plugin.

Where a kind's state is a list of rows and each row publishes what may be done to it, an action is refused unless the row it names also offers that verb. Naming a row is not enough on its own: a row standing for a session with no open tab carries a recorded name that an unrelated tab may currently share, so authorising by name alone would let a plugin close a tab a row never offered to close.
...
The image, PDF, video, audio, page, markdown, schedules, and sessions plugins declare their intents as one table (`defineIntents`), which checks every request the same way before any plugin code runs. The markdown table is empty, so every intent sent to a markdown tab is rejected as unknown. The conversations plugin still checks its own intents, because which ones it accepts depends on whether the tab is the list or a single conversation. The tab payload must pass the plugin's own guard, or the plugin is disabled with `invalid <id> tab payload`. The intent name must be one the table itself declares, or the request is rejected with `unknown <id> intent "<name>"`. A name that merely exists on every object, such as `toString`, is an unknown intent too, not a crash. The payload must pass that intent's guard, or the request is rejected with `invalid <intent> payload`.
