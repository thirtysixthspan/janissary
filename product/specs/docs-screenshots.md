...
`./scripts/run.mjs docs-screenshots` regenerates the screenshots the pages under `documentation/user-documentation/` reference. Passing shot names captures only those; passing a name no entry declares fails with `No manifest entries match: <names>`. That is the spelling for a contributor working inside a Janissary checkout; the unattended task playbook reaches the same script as `$janissary/scripts/run.mjs docs-screenshots`, because a task prompt runs against whatever project its tab is open on rather than against a checkout of Janissary (see [[task-picker]]). Each shot is declared once in `scripts/docs-screenshots/manifest.mjs`, and the entry's name is its output filename, so a page's `/screenshots/<name>.png` path stays stable across reruns. The homepage hero, `demo.png`, is not a manifest shot and is never regenerated.
