...
- **No opener for the extension** — only the inline presentation needs one. `open <file>` reports
  ...
  is held in the notification queue and shown as a toast when no feed is on screen (see
  [[notifications]]), so the report is visible whether or not the user had the feed up.
