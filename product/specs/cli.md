...
`janus init [<project-dir>]` runs attached, printing straight to the terminal, the same as `stop`. It creates the standard `ai/` and `product/` directory tree (`ai/guidelines`, `ai/personas`, `ai/tasks`, `product/backlog`, `product/plans/draft`, `product/plans/ready`, `product/plans/complete`, `product/plans/deferred`, `product/specs`) in the target directory (current directory by default), creating parent directories as needed. `product/backlog/` is seeded with the six standard backlog files (`bugs.md`, `chores.md`, `documentation.md`, `features.md`, `issues.md`, `technical-debt.md`), each containing the standard `ready`/`development`/`deferred` section structure with empty sections. Every directory that is still empty once the tree and backlog files exist gets a `.gitkeep` file so git tracks it. Running `init` again against a directory that already has some or all of the scaffold in place is safe — existing directories, backlog files, and their contents are left alone. It prints the list of directories created and exits 0.
