...
Shell commands (the `shell` keyword, stripped) are written to the tab's persistent shell via stdin. The command runs inside a group with stderr redirected to stdout, and a unique delimiter (`__JS_END_<tab>_<timestamp>__`) is echoed after it to mark the end of output.
...
- All keyboard input — including `Ctrl+C`, `Ctrl+D`, `Ctrl+Z` — is forwarded to the PTY. Only the tab-switch chord (`Shift+←/→` and `Cmd+Shift+[/]`, the same definition the window handler switches tabs on) bubbles out to the window handler.
...
- Shift+drag selects text with the terminal selection layer exactly as it does in a harness tab — see Selecting and copying terminal text in `harness.md`.
...
A command that runs in a PTY — whether promoted by detection, forced with `--pty`, or opened as an inline terminal card — is launched through an interactive shell, so unlike the tab's own shell (see Shell startup files) it does read the user's startup files and has the `PATH` they set. Nothing they print is a problem there: the bytes go to a real terminal, exactly as they do when the user opens one themselves. It is deliberately not a *login* shell: login startup re-runs the system's `PATH` setup, which would reorder the `PATH` janissary inherited from the user's terminal and so could resolve a bare command name to a different copy than the user's own terminal does. Only `bash` and `zsh` are started interactively; any other shell is given the command alone rather than risking an option it does not accept.
