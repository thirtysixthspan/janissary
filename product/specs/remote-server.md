...
- **Leftover that cannot be removed:** nothing is provisioned, the placeholder closes at once, and `Cannot launch "<name>": could not remove leftover workspace "<name>" on <host> (<path>) — <reason>.` is posted.
