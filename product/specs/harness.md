...
A browser that dies after it started is replaced rather than lost. Janissary relaunches it on the
same endpoint, under the same two secret paths, keeping the tab's guard, ports, and scratch directory
exactly as they were — so the `JANISSARY_BROWSER_WS_ENDPOINT` the harness was handed at spawn goes on
working and nothing has to be pushed into a process that is already running. A tab gets three
replacements over its whole life, counted for the tab rather than reset by a browser that ran for a
while, because a browser dying over and over is broken in a way relaunching will not fix. Each
replacement makes up to five attempts to bind, a quarter second apart, since the browser that just
died may still be releasing the port.

A replacement is not a failure and is not reported: no notification, no band on the tab, and the
metadata row's browser flag stays lit. What is lost is any connection open at that moment, because
its pages died with the process. The endpoint answers again as soon as the replacement binds, so the
agent's ordinary one-retry connect reaches the new browser.

When the browser is gone for good — a launch that failed, a guard that died, a replacement that could
not start, or a fourth death after the three replacements are spent — the report is delivered twice: a
line in the notifications tab naming the tab it belonged to, and the same text on the `-b` tab itself,
in a band above its terminal where a failed workspace clone already
...
it. The tab keeps running — only its browser is gone. Nothing restarts it after that point; a later
attempt to connect simply fails. Closing a `-b` tab stops its browser and removes its scratch
...
Every replacement writes its own line into that same output, so a tab that burned through its three
replacements reports the history along with the failure rather than the bare fact that it ended.
