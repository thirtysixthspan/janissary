...
4. **Approval keystroke for claude is `"\r"`; `y` alone does NOT work.** The feature says "an approval string 'y'", but every captured claude gate (see "Ground truth" below) is a **numbered selection menu**, not a `y/n` prompt — it offers `❯ 1. Yes / 2. Yes, and always allow… / 3. No` with option 1 highlighted. Sending the literal `y` would not approve it. **The keystroke is `"\r"`** — Enter accepts the highlighted `❯ 1. Yes`, and (per the four captures) this one keystroke works across every claude gate variant regardless of question/footer/option wording. It is preferred over `"1\r"` so it keeps working if option ordering ever changes, as long as the default stays "Yes". (The keystroke is stored per-harness in the detector table so opencode/codex can add their own later, but only claude's is populated now.)
...
Four claude gate variants were captured live (Bash in-project, Bash out-of-project, Fetch, MCP tool). The important lesson across them: **the question line, the footer, and the option-2/3 wording all vary by tool type, but the numbered `❯ 1. Yes` … `3. No` menu is invariant, and `\r` approves every variant.** So detection should key on the menu *structure*, not the question or footer text, and the keystroke is a single `"\r"` regardless of variant.
...
### Detection strategy (revised from the four captures)

- **Primary anchor — the menu structure, not the question or footer:** require the highlighted-default line `❯ 1. Yes` **and** an option-3 line whose text begins with `3. No`. This pairing is present in all four variants and is independent of the question, footer, and option-2 wording, so it survives every difference observed (Bash/MCP say `Do you want to proceed?` but Fetch does not; Bash has a `· ctrl+e to explain` footer but MCP/Fetch do not; option 2 even rendered as `2.Yes` with no space in the MCP case). Matching rules, decided: trim each line and prefix-match (`❯ 1. Yes` may be followed by more text; `3. No` matches `3. No, and tell Claude…`); tolerate a missing space after the option number (`3.No`), since the MCP capture proves the renderer can drop it (`2.Yes`); the `❯` glyph is the highlight marker and is required on the option-1 line.
- **Bottom-of-screen window + ordering, to kill quoted-text false positives:** both anchor lines must fall within the **last 10 lines** of the capture (trailing blanks are already dropped by `captureNow`), with the `❯ 1. Yes` line **before** the `3. No` line. In all four fixtures the menu occupies the final ~6 lines. This prevents the classic false positive: a claude session merely *printing* gate-shaped text (quoting a captured fixture, catting this very plan) scrolls that text up the screen while the real input line stays at the bottom — such a screen must not trigger injection.
...
- **Still uncaptured:** claude's **edit-approval** gate (Write/Edit are auto-accepted in this session's mode, so it couldn't be surfaced) and any **plan-mode** prompt. Both must be captured and checked against the `❯ 1. Yes` + `3. No` anchor before shipping, in case their structure differs.
...
- **Detection ground truth for claude is already captured** (see Ground truth — four variants gathered live). The same `harness capture <label>` (`harness-manager.ts:50-59`) path is how opencode/codex would be captured if/when they're added later, but that's out of scope here.
...
1. Ground truth for claude is **already gathered** (see Ground truth — four variants captured live; the detector anchors on `❯ 1. Yes` + `3. No`, keystroke `"\r"`). Before shipping, capture claude's still-unseen **edit-approval** and **plan-mode** variants (needs a session where edits aren't auto-accepted) and confirm they carry the same `❯ 1. Yes` + `3. No` structure; if a variant differs, widen the anchor accordingly. (opencode/codex are out of scope — Decision 0.)
...
- Manual end-to-end (the decisive check — detection accuracy can't be proven by tests alone):
  - `harness claude -w -y`, `notifications` open in a dock; drive the harness to a real permission prompt (e.g. ask it to run a shell command that its CLI gates, and a Fetch, and an MCP tool call, to exercise all three captured gate shapes); confirm each gate is dismissed automatically within ~1-2s and a `claude: Auto-approved a permission prompt` line appears in the feed.
