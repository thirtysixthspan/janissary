...
Closes the current tab and all of its associated connections — its shell, ACP session, browser, harness/interactive terminals, and scheduled timers — removes any workspace clone and its in-memory agent state, and selects an adjacent tab. `close` (or its alias `exit`) is reserved for closing tabs, not exiting the app — exiting the app is `quit`, which asks for confirmation first (see `quit-confirmation.md`). Closing the last remaining tab opens a fresh `janus` tab, exactly as on launch. `close page <n>` (or `exit page <n>`) closes a numbered page tab instead of the active one. (Open SQLite connections are global, not tab-scoped, so they are left open — close them with `connection close sqlite:<name>`.)
