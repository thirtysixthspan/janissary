...
Full form: `open [external] [page] <target>`. Relative paths resolve against the tab's working directory. What kind of tab you get depends on the file type — see [Image viewer](/tab-types/image-viewer), [Markdown preview](/tab-types/markdown-preview), and [Embedded web pages](/tab-types/web-pages). Every tab it opens joins the current tab's [group](/getting-started/groups) and can be closed from the strip with its × button.
