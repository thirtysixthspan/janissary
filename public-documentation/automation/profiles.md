...
`profile launch <name>` opens a tab for every entry in the profile: agents come back with their transcript, history, working directory, and schedule; harnesses launch fresh with whatever model, directory, workspace flag, and startup commands the profile specifies. All the launched tabs land in one new [group](/getting-started/groups), so the profile reads as its own colored band in the strip. `profile list` names the profiles you have; a name that doesn't exist gets `No profile named "<name>".`
...
- **`workspace`** — launch in a fresh [workspace clone](/advanced-agents/workspaced-agent), like `-w`.
- **`cwd`** — starting directory.
- **`run`** — commands typed into the harness once, shortly after launch.
- **`schedule`** — timers in the [`schedule` grammar](/automation/scheduling), minus the leading `schedule` keyword and any `in <tab>` clause (each line belongs to this tab). A line that doesn't parse is reported at launch and skipped.
