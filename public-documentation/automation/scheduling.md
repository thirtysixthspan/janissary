...
In an agent tab, the command is dispatched as if typed. In a [harness tab](/advanced-agents/harness), the command is typed into the harness as a line of input — and if the harness isn't accepting input yet, the timer stays due and retries until it lands. After firing, a one-shot timer is removed; a recurring one advances to its next run.
...
[Profiles](/automation/profiles) can pre-author a harness tab's schedule so it's set up on every launch.
