...
Ask for one with `--workspace` (or `-w`) when creating an [agent](/advanced-agents/workspaced-agent) or a [harness](/advanced-agents/harness). Two properties define it — disposability and isolation.
...
- SSH doesn't work from inside, which is why pushing to GitHub needs a token — see [Workspaced agents](/advanced-agents/workspaced-agent).
