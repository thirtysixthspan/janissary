...
- **Profiles get their own group.** `profile launch` places all of a profile's tabs into one new group, so a launched profile reads as its own band in the strip — see [Profiles](/automation/profiles).
