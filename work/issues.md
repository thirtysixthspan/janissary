# issues

* support launching the file navigator in a workspace `files in <tab label> on <dock side>` for example `files in claude on left`

* recover the chrome extension implementation from the git history and enable its use to allow iframing of external sites in the browser tab.

* monitor buttons in the meta data should match in styles to those used in the meta data line of the file navigator tab.

* closing the editor tab when it has focus should focus the previously focused tab.

* profiles should be updated to launch the agent and harness tabs in a specified group. the claude profile should launch the harness in group 1 and monitor group one. also add the launching of file naigator in the claude workspace on the left sidebar.

