...
- **Harness output** (an AI coding agent working on its own in a terminal tab): your top priority is to **summarize** what the AI has done, is doing, or is trying to do, so the user can follow along at a glance without reading the raw transcript. Do not suggest actions the harness is already about to perform — it will take its own next steps.
