...
It stops and leaves the PR open if the check gate is red, if the PR conflicts with `master` (resolve those with `pr-rebase.sh` — see Step 7 of `ai/tasks/workspace/merge-change-to-master.md`), or if a check fails.
