...
Two commands still escape this. `harness` and `ssh` have no entry in `coreCommands`; `CommandManager.run` matches them with inline regular expressions ahead of `resolveCommand` and calls their managers itself, in two branches with byte-identical bodies. Every other dispatcher — the capture path included — consults only the registry, so those two are unknown commands there. That is the standing exception to this principle, not a mechanism to imitate.
