...
Per-tab state used to be scattered across a dozen label-keyed maps on `Controller`. It is now on the tab itself: `Tab` in `src/tab/types.ts` carries the `view` discriminant and its five payloads (`harness`, `editor`, `files`, `plugin`, `monitor`), the transcript log, the command history, and a `runtime` sub-record holding `cwd`, `busy`, `context`, and `queue`. `TabManager` (`src/tab/manager.ts`) owns those records; `src/tab/runtime.ts` reaches the runtime sub-record, and the predicates in `src/tab/view-guards.ts` narrow a payload instead of recovering it with a non-null assertion. `Controller` itself declares only `managers` and `projectDir`.

The maps did not disappear, though — they moved onto the feature manager that owns each concern (`PseudoterminalManager`, `AcpManager`, `ScheduleManager`, `ConnectionManager`, `ConversationsManager`, and the rest of `Managers`), each still keyed by tab label. That is one owner per *resource*, which is a real improvement over a dozen on one class, but it is not yet one owner per *agent*: closing a tab still means every one of those managers being told, and a new per-tab concern still tends to arrive as another map. The actor model is the antidote: encapsulate one agent's state behind one owner, mutated only through well-defined methods.

**Rule.** New per-agent state belongs *on the tab record its manager owns*, not in a new `Map<label, …>` on a manager or the controller. An agent owns its shell, PTYs, ACP session, browser, schedule, and connections; nothing reaches into those from outside except through its methods.

## 3. The controller dispatches; feature services execute

`controller.ts` was once a ~1130-line God Object holding command routing, shell I/O, PTY management, ACP loops, browser glue, scheduling, messaging, globbing, file serving, and persistence in one class. It is now 113 lines and does none of that work itself: every method forwards to a manager on `Managers` or to a feature module (`controller/transcript.ts`, `controller/completion.ts`, `state-event.ts`). Most of its surface is not even written there — five adapter factories assembled in the constructor supply it (`controller/tab-adapter.ts`, `monitor-adapter.ts`, `editor-adapter.ts`, `file-navigator-adapter.ts`, `plugin-adapter.ts`), and those are what reach `controller/file-navigator.ts`, `editor/save.ts`, and `monitor/window.ts`. Self-contained command logic lives in `src/commands/*.ts` per principle 5.

The pressure did not disappear — it moved. No file under `src/` or `web/src/` carries an `/* eslint-disable max-lines */` suppression; the rule is switched off in `eslint.config.mjs` for test files only. The largest non-test sources are `src/file-navigator/manager.ts` (296 raw lines), `src/sandbox/index.ts` (290), and `src/plugins/api.ts` (289) — those, not the controller, are where this principle is under the most pressure. None is actually over the limit, because `max-lines` counts with `skipBlankLines` and `skipComments`, and all three are heavily commented; raw size is the early warning, not the violation.
...
Each command has exactly **one** definition, and every command runs the same way. A `Command` (`src/commands/types.ts`) carries `name`, `match`, and `run(command, { label, index }, managers)`, and owns its logic in `src/commands/<name>.ts`; the registry in `src/commands/index.ts` lists them, and `CommandManager.executeCommand` (`src/command/manager.ts`) looks a resolved name up there and awaits its `run`. There is no second execution path and no per-command context object: the third argument is the `Managers` registry, so a command needing tab, PTY, or session machinery reaches its owner directly. The earlier shadow system — vestigial `handler` functions plus a 40-field `CommandHandlerContext` describing a React-hook world that no longer existed — has been deleted.
...
Agent messages (`msg … command`/`request`) enter through `CaptureManager.run` (`src/capture/manager.ts`), which classifies text with the same `resolveCommand` and shares the unknown-command route recognition (`recognizeRoute` in `src/route-choice.ts`) with the command bar. What still differs is execution, because a message owes its sender captured text: a `shell` resolution runs piped with `detect: false` and anything interactive is refused, a command with an optional `capture` hook on `Command` (`acp`, `browser`) answers through it, and every other command runs through `executeCommand` with its reply read back from the tab's last new log entry. That last-entry read is the remaining seam — a `run` that returned its output would close it.
