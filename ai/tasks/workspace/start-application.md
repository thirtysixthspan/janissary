...
**Those two facts together decide how a caller drives the app, and it is not the shape the manual's "Keeping it alive" section suggests.** One connection is one session, and the app lives only while a client of *that* session is connected, so **a background holder does not keep the app alive for the next script.** Measured in a find-bugs run: a holder's page was a healthy client on its own, a second driver's `connect()` left it holding nothing, its heartbeat stopped, and the app exited on its own about a second later — so the next driver found `ERR_CONNECTION_REFUSED`. The working shape is the opposite of a holder: **one connection per batch of work, held open from the first interaction to the last, and a second batch means a second app instance.** Fold the work into one driver where you can; where you cannot, expect to run the start command again, and note the new address and pid.

**Driving the app is `./scripts/run.mjs e2e-driver <driver.mjs>`, and the driver is one batch.** It takes the address from the app's own log rather than from a command line, so no token is passed, pasted, or written down:

```bash
./scripts/run.mjs e2e-driver ./temp/find-bugs/viewer.mjs \
  --log ./temp/find-bugs/work/.janissary/log/server.log --scratch ./temp/find-bugs --out viewer
```

A driver is a module with a default export taking `{ page, record, out, shot, press, runCommand, tabs, tabSummary, dialogs, notifications, watchRequests, viewer, editor, canvasSignature, consoleLog }`: `page` and `logs` from the session, the DOM readers from `scripts/e2e/inspect.mjs` and `scripts/e2e/inspect-image-tab.mjs`, `out(name, value)` and `shot(name)` for evidence under `--scratch`, and `record` — the object the driver fills in and the runner writes to `<scratch>/<out>.json` **whether the driver returns or throws**, so a driver that dies at its last step still leaves every earlier observation on disk. The runner never calls `browser.close()`: it closes its own page and context and lets the connection drop, which is also what lets the app exit on its own. `./scripts/run.mjs e2e-fixtures <directory>` writes image fixtures (landscape, portrait, square, tiny, an SVG) into a directory the run owns, for a caller that has to open real image files.

Three things about the app's DOM are worth handing a driver writer rather than letting them rediscover them. A view tab's body has no command bar, so a command meant for one is typed from an agent tab — `runCommand` makes that switch itself. Every plugin view stays mounted while its tab is hidden (`web/src/MountedViewLayers.tsx`), the hidden ones under `display: none`, so scope to `.tab-body:visible` and never to the first `.plugin-tab` in the document. And a plugin payload is refreshed when the server re-registers a file, so a size or a reference in the header can change under a driver that only looks once.
