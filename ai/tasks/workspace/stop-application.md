...
**The app may already be gone, and that is ordinary.** Its lifetime follows its websocket clients: the last one disconnecting makes it broadcast `bye` and exit about a second later (`src/index.ts:93-99`), and closing the last remaining tab quits it outright (`src/tab/close.ts:30`). A run that lost its browser, or closed the last tab, leaves an app that stopped itself — which the stop command reports and Step 1 already calls a successful stop. **A caller driving the app through `./scripts/run.mjs e2e-driver` is the ordinary case of exactly that.** One connection is one session, the runner closes its own page and context when the driver finishes, and the app then quits by itself: `no running janus instance for <dir>` is what a completed run leaves behind, not a sign that anything went wrong. The corollary for a run that needs a second batch of work is that it starts the app again, which takes a new port and a new pid — and the address a driver reads is the one in the newest `__JANUS_URL__` line of the log.

**A driver still running when this task is called is not a process to signal.** `./scripts/run.mjs e2e-driver` (`scripts/e2e/session.mjs`) holds one browser connection for the length of one driver and gives back only its own page and context; it never calls `browser.close()`, and the browser belongs to the tab besides. Stopping the app under it makes its next interaction fail, which is the correct outcome and needs no escalation: this task signals the app through the lock's pid, or not at all.
