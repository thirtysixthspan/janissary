...
```bash
$janissary/scripts/run.mjs pr-commit "refactor: extract parseSpec() helper to cut loadConfig cognitive complexity" \
...
```bash
$janissary/scripts/run.mjs pr-create-pr "$OWNER_REPO" "$BRANCH" "<title>" ./temp/pr-body.md
```
...
```bash
$janissary/scripts/run.mjs pr-check-mergeable "$BRANCH" "$OWNER_REPO"
```
...
```bash
$janissary/scripts/run.mjs pr-rebase origin my-branch-name
```
...
```bash
$janissary/scripts/run.mjs pr-merge "$BRANCH" "$OWNER_REPO"
```
