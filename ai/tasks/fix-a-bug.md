...
4. **Replicate the reported failure** before writing any fix, so you can watch it fail:
   ...
   - For a browser-visible bug in a tab launched with `-b`, use the attached browser rather than trying to launch Chromium from the workspace. Confirm `JANISSARY_BROWSER_WS_ENDPOINT` and `JANISSARY_PLAYWRIGHT` are both set, connect with the client at `JANISSARY_PLAYWRIGHT`, and keep that browser open. Start a separate Janissary instance from the workspace with `node bin/janus.mjs --no-open <project-dir>` and reproduce against that instance — never drive the human's live Janissary session. Follow [`ai/guidelines/sandbox-e2e-browser.md`](../guidelines/sandbox-e2e-browser.md) for connection, navigation, and lifecycle constraints.
