...
For a web app, drive the app through `./scripts/run.mjs e2e-driver <driver.mjs> --log <project-dir>/.janissary/log/server.log --scratch ./temp/find-bugs/ --out <name>`, which opens the connection, the context, and the page, reads the address off the app's own log so no token is ever written down, and writes what the driver recorded to `<scratch>/<name>.json` whether the driver returns or throws. Keep that one connection open for the whole batch of behavior it exercises, including root-cause research: `ai/guidelines/sandbox-e2e-browser.md` is the manual for the endpoint, and it is also where the trap is. Every `chromium.connect()` is its own session with its own browser, and the app quits about a second after its last client disconnects, so **a background holder does not keep the app alive for the next script** — a second connection leaves the holder holding nothing, and the app exits on its own before the next driver can reach it. What works is the opposite of a holder: one connection per batch, held from the first interaction to the last. Fold as much of the checklist as you can into one driver; where a second batch is genuinely needed, run Step 3's start command again against the same project directory and take the new address, keeping the record's address, pid, and port current.
...
Read each selected spec in full. Derive a bounded checklist of its user-visible normal behavior, edge cases, and errors, noting the exact spec sentences that define expected results. Prepare existing content through scratch files or the project's seed/fixture commands when needed; `./scripts/run.mjs e2e-fixtures <scratch work dir>` writes image files (a landscape, a portrait, a square, a tiny one and an SVG, each with distinguishable corners) when a selected spec is about images and the project ships none. Setup can use files directly; the behavior under test must go through the browser or tool. The driver's readers save the selectors this app needs: `runCommand` switches to an agent tab when the tab on screen has no command bar, and everything measured is scoped to `.tab-body:visible` because a hidden plugin view stays mounted under `display: none`.
