...
Research tasks write that sentence deliberately and in exactly that form — [`find-namespaces.md`](research/find-namespaces.md) for [`improve-namespacing.md`](hygiene/improve-namespacing.md), [`find-complex-code.md`](research/find-complex-code.md) for [`reduce-complexity.md`](hygiene/reduce-complexity.md), [`find-packages-to-update.md`](research/find-packages-to-update.md) for [`update-package-outside-range.md`](hygiene/update-package-outside-range.md). Wording may vary slightly if a human wrote the entry by hand; what must be present is an unambiguous reference to a specific playbook file, plus the target. Take it at its word and use the task it names.
