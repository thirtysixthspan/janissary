...
- `documentation/public/screenshots/*.png` (generated output of `$janissary/scripts/run.mjs docs-screenshots` — commit the PNGs the capture produces; never hand-edit them)
...
4. Capture it: `$janissary/scripts/run.mjs docs-screenshots <name>` — and pass, in the same run, the name of any existing shot whose page you also edited and whose UI may have moved, so those stay fresh too.
   ...
   - If it reports Playwright Chromium missing (normal in a sandboxed workspace launched without `-b`, where the browser download is blocked and there is no attached browser to drive instead), or a shot is skipped for a missing `requiresBinary` binary, **do not try to install anything** — ship the page without the screenshot. If you added a `data-doc-shot` attribute for this shot, keep the attribute (it's harmless, and the next run can use it) but drop the manifest entry, and note it in your Step 7 report.
