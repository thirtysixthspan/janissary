...
Janissary's editor-plugin API is for trusted plugins bundled in this repository. An editor plugin binds a keyboard chord in the editor tab and answers with the edits to make and where to leave the selections. It cannot be installed at runtime and is not sandboxed.
...
The yielded chords are Tab, but only while the selection spans more than one line; Shift+Tab, always; and Escape, but only while more than one selection is active — see `yieldsToPlugins` in `web/src/editor/keys.ts`. They are offered to plugins first; if no plugin claims one, the core action runs anyway, so Tab keeps inserting a tab character and Escape keeps collapsing the selection when the plugin that binds them is disabled. The list is a hard-coded table in the core key file: a plugin cannot add to it, because delegating a core binding is a decision the editor makes about itself.
...
type EditorPluginRequest = {
  command: string;
  file: string;
  selections: readonly { anchor: Pos | null; cursor: Pos }[];
...
`selections` is every selection the editor holds, in creation order with the primary — the most recently created caret — last, and it is never empty. A handler that thinks in terms of one selection reads `primarySelection(request)`; a handler that creates or drops carets reads and answers with the whole list.
...
The optional `selections` replaces the editor's whole selection set, in creation order with the new primary last; omit it to leave the set as it is. It describes where the carets should sit *after* the edits, so it may name a column the pre-edit line was too short to hold. An explicitly empty list is refused — an editor always has at least one caret.
...
An answer is checked against the buffer before any of it is applied. A position outside the document, a range that ends before it starts, two edits covering overlapping ranges, an empty selection list, or two selections covering overlapping ranges refuses the whole result: nothing is applied, no undo step is recorded, and the plugin is disabled. Ranges that merely touch at a boundary are fine.
...
A plugin is handed data and returns data. It receives no capability object, no client, and no file access, and it may change only text and selections — never a save, a scroll, a rename, a close, an open, another tab, or anything it draws itself.
