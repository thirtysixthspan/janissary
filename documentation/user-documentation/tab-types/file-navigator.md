...
Anywhere the header shows a branch name, it also shows a **Commit changes to origin** button right after the Pull from origin button. Hovering names the branch your commit will push to (`Commit changes to origin (branch <name>)`) plus its current state. Clicking it commits and pushes everything that has changed in the tree's repository. To commit just some of it, right-click a row and choose **Commit to origin** — that commits the selected file or files, or everything inside a folder if you right-click one.
