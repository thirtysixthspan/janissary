...
Files the pull added appear and files it removed disappear, at the top level and inside every folder you had open — and the folders you had open stay open. A folder only closes on a refresh when it is really gone from the folder that held it.
...
The new tab's name is highlighted in the editor's metadata row so you can type over it right away. Enter accepts the typed name (it becomes the filename, with no extension added) and updates both the tab and its metadata; Escape keeps the default name even if you had typed something, and either way focus returns to the buffer. You can still rename by editing the tab label instead — the typed text becomes the filename with no extension added. Saving writes to that name; if another new-file tab already saved to it first, your save silently falls back to the next free name instead of overwriting it.
...
| `Enter` / `Space` | Open a file, toggle a directory, or (on `..`) go up — a Markdown file opens in the editor, as double-clicking it does |
| `Shift+Enter` | Edit the selected file, exactly as `Shift`+double-click does — a Markdown file opens its preview and a video or audio file goes to your system's player |
