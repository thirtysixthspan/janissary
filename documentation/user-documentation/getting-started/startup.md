...
| `syncPaths` | `["product/backlog/", "product/plans/"]` | Project-relative paths kept synced with GitHub in the [editor](/user-documentation/tab-types/editor#keeping-a-file-synced-with-github). An entry is an exact file path, a directory path ending in `/` (every file under it, at any depth), or a `*` wildcard standing in for one path segment. There's no runtime command for this; edit the file directly. |
